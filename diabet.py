...
def women():
    ...
    def process_all_inputs():
        ...
        # preds = xgb_clf.predict(xi)
        # print(preds[0])
        pish = preds[0]
        label1 = ['شما خطر ابتلا به دیابت دارید']
        label2 = ['شما خطر ابتلا به دیابت ندارید']
        ...
        if pish == 1:
            ...
            for i, item in enumerate(label2):
                label = tk.Label(root, text=item)
                label.grid(column=1, row=9)
...
def men():
    ...
    labels = ["قد", "وزن", "فشار خون", "قند خون", "سن", "گلوکز خون", "وراثت دیابت", "ضخامت پوست"]
    entries = []
